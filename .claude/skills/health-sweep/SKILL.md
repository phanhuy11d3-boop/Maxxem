...
3. **db-auditor** — "Audit DB and outbox state machine: run `py -3 scripts/diagnose_telegram.py`, `py -3 scripts/audit_agent.py`, `py -3 scripts/query_recent_non_neutral.py --limit 10 --max-age-minutes 30`. Report counts of pending/sent/failed/expired and any anomaly."
